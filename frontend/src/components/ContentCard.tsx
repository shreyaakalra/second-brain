  ...
  return (
    <div className="bg-white border-2 border-black rounded-2xl p-5 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col gap-3 group hover:-translate-y-1 transition-transform duration-200 ">
      ...
      <div className="flex items-center justify-between">
        <span
          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-black border border-current ${typeColors[card.type]}`}
        >
          {typeIcons[card.type]}
      ...
      {card.previewImage && (
        <div className="inline-block">
          <img src={card.previewImage} className="block max-w-full h-auto" />
        </div>
      )}
      ...
      {card.tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-auto">
          {card.tags.map((tag) => (
            <span className=" px-2 py-0.5 bg-neutral-100 text-neutral-600 text-xs font-bold rounded-full">
              #{tag}
            </span>
          ))}
        </div>
